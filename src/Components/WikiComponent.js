...
// Enthält die Gestaltung des Wikis
  ...
  return (
    <div className="wiki-container">
      ...
      <ul className="WikiLinks">
        ...
        <li onClick={() => handleInfoClick('Die <strong>Standardabweichung</strong> ist ein Maß für die Streuung um den Mittelwert. Er gibt Auskunft darüber, wie stark die Werte im Durchschnitt vom Mittwelwert abweichen.')} className="clickable-item">Infos über Standardabweichung</li>
        ...
        <li onClick={() => handleInfoClick('Mithilfe von <strong>Diagrammen</strong> werden Daten graphisch dargestellt. Dabei gibt es verschiedene <strong>Visualisierungsformen wie Balken-, Säulen- und Tortendiagramme, Liniendiagramme, Histogramme, Boxplots und Scatterplots</strong>.<br><br> <strong>Balken-, Säulen- und Tortendiagramme</strong> stellen absolute oder relative Häufigkeiten dar: Je höher ein Balken oder größer ein Tortenstück ist, desto höher der Wert der Variable. Ein <strong>Liniendiagramm</strong> bildet einen Wert mit seinem zeitlichen Verlauf ab. Die Datenpunkte sind durch eine Linie verbunden, weshalb ihre Entwicklung gut ablesbar ist. Während diese Diagramme die Daten in diskreter Form (einzelne, getrennt voneinander existierende, meist ganzzahlige Werte, wie die Anzahl der Bücher in einem Regal) darstellen, beschreibt ein <strong>Histogramm</strong> Daten kontinuierlich (beliebiger Wert in bestimmten Bereich eines Spektrums, wie Zeit: zwischen jedem gemessenen Wert können unendlich viele weitere Werte existieren). Ein <strong>Histogramm</strong> veranschaulicht die Häufigkeitsverteilung von Daten in Klassen. <br><br>Ein <strong>Scatterplot</strong> hingegen visualisiert den Zusammenhang zwischen Daten. Er stellt ein Wertepaar für zwei Variablen dar. Wenn es einen hohen Zusammenhang zwischen diesen zwei Variablen gibt, sind die Punkte im Plot auf eine bestimmte Weise angeordnet. Ein positiver Zusammenhang (hohe Werte Variable A + hohe Werte Variable B) wird durch eine steigende Gerade dargestellt und ein negativer Zusammenhang (hohe Werte Variable A + niedrige Werte Variable B) durch eine fallende Gerade. Besteht kein Zusammenhang zwischen den Variablen, wird keine Gerade eingezeichnet und es ist kein Muster der Punkte zu erkennen und sie wirken zufällig im Plot verteilt.<br><br> Ein <strong>Boxplot</strong> kann man als eine visuelle Zusammenfassung von Daten beschreiben. Er stellt neben der Verteilung der Daten zusätzlich die Quantile und den Median (Box mit Linie in Box) und den minimalen, bzw. maximalen Wert (Antenne/Whiskers) dar. Ausreißer werden mithilfe des Kreises beschrieben. Ein Ausreißer ist eine Beobachtung, die von den üblichen Werten entfernt ist. Da sie durch Zufall oder auch oft durch Messfehler entstehen, möchte man sie von der Analyse ausschließen, um das Ergebnis nicht zu verfälschen. ')} className="clickable-item">Infos über Visualisierungsformen</li>
